...
class Email extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('view', function ($row) {
                    return '<button class="btn text-lblue btn-modal" data-container=".customer_modal" data-href="' . route('viewemail', ['id' => $row->id]) . '">View</button>';
                })
                ...
                ->addColumn('action', function ($row) {
                    return '<button class="btn btn-modal" data-container=".customer_modal" data-href="' . action([Email::class, 'edit'], [$row->id]) . '">
        ...
        return view('email/index')->render();
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('email/create', compact('domainmaster'))->render();
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'emailserver' => 'required|string|max:255',
            'noofemailid' => 'required|integer',
        ...
        $existingDomain = DB::table('emailserver')
        ...
        $data = [
            ...
            'noofemailid' => $request->noofemailid,
        ...
        DB::table('emailserver')->insert($data);

        session()->flash('secmessage', 'Email Server Details Successfully Added.');
        return response()->json(['status' => 1, 'message' => 'Email Server Details Successfully Added.'], 200);
    }

    public function edit($id)
    {
        $email = DB::table('emailserver')->select('id', 'company_name', 'domainname', 'dateofregis', 'dateofexpire', 'dateofrenewal', 'empid', 'emailserver', 'noofemailid', 'vendorname')->find($id);

        $accounts = DB::table('accounts')->select('id', 'company_name')->find($email->company_name);

        $domainmaster = DB::table('domainmaster')->select('domainname')->find($email->domainname);
        // dd($domain,$accounts,$domainmaster);
        return view('email.edit')->with(compact('email', 'accounts', 'domainmaster'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Extracting month and year for domain month
        $thisMonth = date("M", strtotime($dateofexpire));
        $thisYear = date("Y", strtotime($dateofexpire));
        $domainmonth = "$thisMonth-$thisYear";
        ...
        // Update the email server record
        $updated = DB::table('emailserver')->where('id', $id)->update($data);

        // Fetch user details for email
        $user = DB::table('regis')->where('empid', session()->get('empid'))->first();

        if ($updated) {
            // Sending email notification for domain renewal update
            ...
            Mail::send([], [], function ($message) use ($appName, $request, $founderEmail, $managerMail, $techAdminMail, $bccEmail, $infoMail, $infoMailCom, $user) {
                $message->to($founderEmail)
                    ->cc([$managerMail, $techAdminMail, $infoMailCom])
                    ->bcc($bccEmail)
                    ->from($infoMail, $appName)
                    ->subject("Email Server Renewal")
                    ->html('
                        <html><body>
                                ...
                                <table width="96%" cellpadding="0" cellspacing="0">
                                    <tr><td style="border-top:5px solid #1e96d3;background:#fff;padding:20px;">
                                        <table width="100%" cellpadding="0" cellspacing="0">
                                            <tr><td><p style="font-size:14px;color:rgb(0,0,0);text-align:center">Email Server Renewal Details</p></td></tr>
                                            <tr><td><p style="color:#000;font-size:13px"> <strong>Hi Sir,</strong><br></p></td></tr>
                                            <tr><td style="background-color:rgb(234,234,234);"><p style="font-size:13px;text-align:left"><strong>Email Server Renewal Details</strong></p></td></tr>
                                            <tr>
                                                <td>
                                                    <table style="font-size:12px;width:100%">
                                                        ...
                                                        <tr><td>Next Renewal:</td><td> : </td><td>' . htmlspecialchars($dateofexpire) . '</td></tr>
                                                        <tr><td>Number of Email:</td><td> : </td><td>' . htmlspecialchars($request->noofemailid) . '</td></tr>
    ...
    public function destroy($id)
    {
        ...
        $email=DB::table('emailserver')->select('company_name','domainname')->find($id);

        $domainmaster=DB::table('domainmaster')->select('domainname')->find($email->domainname);

        $accounts=DB::table('accounts')->select('company_name')->find($email->company_name);
        ...
        Mail::send([], [], function ($message) use (
            ...
            $message->to($founderEmail)
                ...
                ->bcc($bccEmail)
                ->from($infoMail, $appName)
                ->subject("Emailserver Removal Details")
                ->html('
                <html><body>
                <table style="background:#efeded" cellpadding="0" cellspacing="0" bgcolor="#EFEDED" border="0" width="575px">
                    <tr><td align="center">
                        <table width="96%" cellpadding="0" cellspacing="0">
                            <tr><td style="border-top:5px solid #1e96d3;background:#fff;padding:20px;">
                                <table width="100%" cellpadding="0" cellspacing="0">
                                    <tr><td><p style="font-size:14px;color:#000;text-align:center">Emailserver Removal Details</p></td></tr>
                                    <tr><td><p style="color:#000;font-size:13px"><strong>Hi Sir,</strong><br></p></td></tr>
        ...
        $upd = DB::table('emailserver')->where('id', $id)->delete();
