...
<div class="row m-0 appac_hide">
    ...
    <div class="profile col-12 col-lg-12 col-xl-12 col-xxl-12 d-flex justify-content-between flex-wrap  align-items-center  p-15">
        <div class="profile-head">
            <h1 class="ch2 comp-name">Expo Customer</h1>
        </div>

        <div class=" justify-content-sm-end d-flex  gap-2 flex-wrap">
            <button class="btn bg-primary text-white ft-15 btn-modal pri-text-color m-0" data-container=".customer_modal" data-href="{{action([App\Http\Controllers\Expocustomer::class,'create'])}}"><i class="fa fa-plus me-1" aria-hidden="true"></i> Add Expo</button>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        var cat_table = $('#example').DataTable({
            ...
            ajax: "{{ action([App\Http\Controllers\Expocustomer::class,'index']) }}",
            columns: [{
                    data: 'sno',
                    name: 'sno'
                },
        ...
        $(document).on('click', '.conformdelete', function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: '/expocustomer/' + Id, // Change this to your endpoint
                        type: 'DELETE',
                        data: {
                            id: Id,
                            _token: '{{ csrf_token() }}',
                    ...
                    window.location.href = '/expocustomer';
                }
            });
        });
