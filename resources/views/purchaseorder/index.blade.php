...
<div class="row m-0 appac_hide">
    <div class="d-flex justify-content-between  align-items-end  inside-nav mb-4">
        ...
        <ul class="nav nav-tabs  my-4  justify-content-end  mb-0  ">
            ...
            <li class="nav-item">
            <a class="nav-link active" href="/purchaseorder"><b>Purchase Order</b></a>
            </li>
            <li class="nav-item">
    ...
    <div class="profile  prof  col-12 d-flex justify-content-between align-items-center  p-15">
        ...
        <div class="col-lg-2 col-sm-12">
            <div class="form-group">
                ...
                {!! Form::select('company_name', $vendorlist, null, ['class' => 'form-select select2', 'required' => true]) !!}
            </div>
        </div>
        ...
        <div class="col-lg-2 col-sm-12">
            <div class="form-group">
                ...
                <select class="d-block form-select" name="paymentstatus">
                    <option value="all" @if(request()->session()->get('purchaseorder_status') == 'all') selected @endif>All</option>
                    <option value="withoutcancelled" @if(request()->session()->get('purchaseorder_status') == 'withoutcancelled') selected @endif>All Without Cancelled</option>
                    <option value="open" @if(request()->session()->get('purchaseorder_status') == 'open') selected @endif>Open</option>
                    <option value="suspence" @if(request()->session()->get('purchaseorder_status') == 'suspence') selected @endif>Suspence</option>
                    <option value="closed" @if(request()->session()->get('purchaseorder_status') == 'closed') selected @endif>Close</option>
                    <option value="cancelled" @if(request()->session()->get('purchaseorder_status') == 'cancelled') selected @endif>Cancelled</option>
                </select>
            </div>
        </div>
        ...
        <div class="col-lg-3 col-sm-12">
            <div class="alidate-input m-b-23 mb-2">
                <p class="fs-4 pt-5 totalamount mb-0">
                    @if(request()->session()->has('totaloppayment'))
                    INR. {{request()->session()->get('totaloppayment')}}
                    @else
                    INR. 0.00
                    @endif
                </p>
    ...
    <div class="profile  prof  col-12 col-lg-12 col-xl-12 col-xxl-12 d-flex  justify-content-between  align-items-center  p-15">
        <div class="profile-head">
            <h1 class="ch2 comp-name m-0" style="white-space: nowrap;">Purchase Order</h1>
        </div>
        <div class="justify-content-sm-end search-bar d-flex w-100">
            <div class="row  form-flex">
                {!! Form::open(['route' => ['purchaseorderaccountsid'], 'method' => 'POST']) !!}
                {!! Form::label('company_name', 'Company Name', ['class' => 'label-color '] ) !!}
                {!! Form::select('accountsid', $vendorlist, null, ['class' => 'form-select', 'required']) !!}
                ...
                {!! Form::close() !!}
                <button class="btn bg-primary text-white ft-15 btn-modal pri-text-color m-0 bcreateview" data-container=".customer_modal" style="display: none;" data-href="{{action([App\Http\Controllers\Purchaseorder::class,'create'])}}"><i class="fa fa-plus me-1" aria-hidden="true"></i> Create </button>
            </div>
        </div>
    </div>
    <div>
        <p class="d-block" style="cursor: pointer; font-weight: 700;" id="pendingstatus">Pending Due ({{$proformadata->total_open}}) - INR. {{number_format($proformadata->total_grosspay ?? 0,2)}}</p>
        <p class="d-block" style="cursor: pointer; font-weight: 700;" id="pendingstatus">Last 90days Pending Due ({{$proformadata90days->total_open}}) - INR. {{number_format($proformadata90days->total_grosspay ?? 0,2)}}</p>
    </div>
                ...
                <table id="example" class="dataTable mt-6 table table-bordered ">
                    <thead>
                        <tr class="bg-white">
                            ...
                            <th class="text-grey">Order No</th>
                            <th class="text-grey">Order Date</th>
...
<script>
    $(document).ready(function() {
        ...
        var cat_table = $('#example').DataTable({
            ...
            ajax: "{{ action([App\Http\Controllers\Purchaseorder::class,'index']) }}",
            columns: [{
                    data: 'sno',
                    name: 'sno'
                },
        ...
        function Oppourtunity(pid = null) {
            ...
            $.ajax({
                url: "{{ route('purchaseorder.index') }}", // Better to use named route if available
                type: 'GET',
                data: {
                    pstatus: id
                },
        ...
        function accountsid(selectElement) {
            ...
            $.ajax({
                url: '/purchaseorderaccountsid', // Change this to your endpoint
                type: 'POST',
                data: {
                    id: selectedValue,
                    _token: '{{ csrf_token() }}',
        ...
        $(document).on('click', '.invoicestatus', function() {
            ...
            $.ajax({
                url: '/purchaseorderstatus', // Change this to your endpoint
                type: 'POST',
                data: {
                    id: Id,
                    inid: inid,
    ...
    $('#searchpaysubmit').on('click', function() {
        ...
        var status = $('[name="paymentstatus"]').val();
        ...
        $.ajax({
            url: "{{ action([App\Http\Controllers\Purchaseorder::class, 'index']) }}",
            type: 'GET',
            data: {
