...
<div class="d-flex justify-content-between  align-items-end  inside-nav mb-4">
    ...
    <div class="profile  col-12 col-lg-12 col-xl-12 col-xxl-12 d-flex justify-content-between flex-wrap  align-items-center  p-15">
        <div class="profile-head">
            <h1 class="ch2 comp-name">Google Sheet</h1>
        </div>
        <div class="justify-content-sm-end d-flex">
            <div class=""></div>
            <button class="btn bg-primary text-white ft-15 btn-modal pri-text-color m-0 " data-container=".customer_modal" data-href="{{action([App\Http\Controllers\Googlesheet::class,'create'])}}"><i class="fa fa-plus me-1" aria-hidden="true"></i> Add Google Sheet</button>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        var cat_table = $('#example').DataTable({
            ...
            ajax: "{{ action([App\Http\Controllers\Googlesheet::class,'index']) }}",
            columns: [{
                    data: 'sno',
                    name: 'sno'
                },
        ...
        $(document).on('click', '.conformdelete', function() {
                ...
                swal({
                    title: "Alert",
                    text: "Are you sure you want to delete the Google Sheet?",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                    ...
                    if (isConfirm) {
                        $.ajax({
                            url: '/googlesheet/' + Id, // Change this to your endpoint
                            type: 'DELETE',
                            data: {
                                id: Id,
                                _token: '{{ csrf_token() }}',
                        ...
                        window.location.href = '/googlesheet';
                    }
                });
            });
