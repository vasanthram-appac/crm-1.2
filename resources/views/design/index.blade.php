...
<div class="row m-0 appac_hide">
    <div class="d-flex justify-content-between  align-items-end  inside-nav mb-4">
        ...
        @if(request()->session()->get('role') != 'user')
            ...
            <li class="nav-item">
                <a class="nav-link active"  href="/design"><b>Design</b></a>
            </li>
            <li class="nav-item">
    ...
    <div class="profile  col-12 col-lg-12 col-xl-12 col-xxl-12 d-flex justify-content-between flex-wrap  align-items-center  p-15">
        <div class="profile-head">
            <h1 class="ch2 comp-name">Design</h1>
        </div>
        <div class="justify-content-sm-end d-flex">
            <div class=""></div>
            <button class="btn bg-primary text-white ft-15 btn-modal pri-text-color m-0 " data-container=".customer_modal" data-href="{{action([App\Http\Controllers\Design::class,'create'])}}"><i class="fa fa-plus me-1" aria-hidden="true"></i> Add Design</button>
        </div>
    </div>
                ...
                <table id="example" class="dataTable mt-6 table table-bordered ">
                    <thead>
                        <tr class="bg-white">
                            ...
                            <th class="text-grey">Design Type</th>
                            <th class="text-grey">Date</th>
                            <th class="text-grey">Description</th>
                            <th class="text-grey">Task Status
                                <select class="d-block" name="design_status">
                                    <option value="">Select</option>
                                    <option value="Open" @if(request()->session()->get('design_status') == 'Open') selected @endif>Open</option>
                                    <option value="Closed" @if(request()->session()->get('design_status') == 'Closed') selected @endif>Closed</option>
                                </select>
                            </th>
...
<script>
    $(document).ready(function() {
        ...
        var cat_table = $('#example').DataTable({
            ...
            ajax: "{{ action([App\Http\Controllers\Design::class,'index']) }}",
            columns: [{
                    data: 'sno',
                    name: 'sno'
                },
        ...
        $(document).on('click', '.taskestatus', function() {
            ...
            $.ajax({
                url: '/designstatus', // Change this to your endpoint
                type: 'POST',
                data: {
                    id: Id,
                    status: status,
        ...
        function Status() {
            var status = $('select[name="design_status"]').val();
            $.ajax({
                url: "{{ action([App\Http\Controllers\Design::class, 'index']) }}",
                type: 'GET',
                data: {
                    status: status
                },
        ...
        $('select[name="design_status"]').on('change', Status);
